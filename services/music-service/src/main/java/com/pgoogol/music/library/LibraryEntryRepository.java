...
public interface LibraryEntryRepository extends JpaRepository<LibraryEntry, Long> {
    ...
    @Query("""
        select new com.pgoogol.music.library.TrackSlotOverride(e.track.spotifyId, e.djSlotOverride)
        from LibraryEntry e
     ...
     */
    @Query("""
        select new com.pgoogol.music.library.TrackDjData(e.track.spotifyId, e.rating, e.djSlotOverride)
        from LibraryEntry e
