...
| **R11** | Nowy `libs/java/llm-starter` — projektowany od zera, **nie** na bazie klienta z music-service. Music-service pozostaje nietknięty; notatka o starterze i o migracji „przy najbliższej pracy nad wzbogacaniem" stoi w jego `docs/DECYZJE.md` jako D40. | Zgodnie z regułą „kod dzielony = starter z `@AutoConfiguration`". Dwa klienty LLM w repo do czasu migracji music — świadomie. |
